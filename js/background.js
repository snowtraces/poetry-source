...
MEM = {
    ...
    cursx: 0,
    lasttick: 0,
    windx: document.body.clientWidth,
    windy: document.body.clientHeight,
    planmtx: [],
};
...
function update() {
    ...
    document.getElementById("BG").innerHTML =
        `<svg id='SVG' xmlns='http://www.w3.org/2000/svg' width='${MEM.windx}' height='${MEM.windy}' viewBox = '${calcViewBox()}'>
         <g id='G' transform='translate( 0, 0)'> ${MEM.canv}</g>
        </svg>`
...
function present() {
    ...
    var step = 1;
    document.documentElement.scrollTo(Math.max(0, pFrame - 10), window.scrollY);
...
for (var i = 0; i < reso / 2 + 1; i++) {
    for (var j = 0; j < reso / 2 + 1; j++) {
        var c = color.r + Noise.noise(i * 0.1, j * 0.1) * 10;
        c -= Math.random() * 20;

        var r = c.toFixed(0);
        var g = (c * color.g / color.r).toFixed(0);
        var b = (c * color.b / color.r).toFixed(0);
